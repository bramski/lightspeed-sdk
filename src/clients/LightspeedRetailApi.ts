...
const searchParamsToQueryParams = (searchParams: SearchParams): Record<string, string> => {
  return Object.entries(searchParams).reduce(
    (queryParams, [param, searchParam]) => ({[param]: toLSQueryParam(searchParam), ...queryParams })
  , {});
};
...
class LightspeedRetailApi {
  ...
  async getToken(): Promise<AccessTokenResponse | never> {
    ...
    const data: FormData = buildAuthFormData(
      this.clientId,
      this.clientSecret,
      this.refreshToken
    );
  ...
  async postCustomer(accountId: number | string, customer: PostCustomer): Promise<Customer | never> {
  ...
  public async getSale(accountId, saleId) {
    const queryString = { load_relations: JSON.stringify(['TaxCategory', 'SaleLines', 'SalePayments', 'SalePayments.PaymentType'])};
    const url = `https://api.merchantos.com/API/Account/${accountId}/Sale/${saleId}.json?${querystring.stringify(queryString)}`;
    const options = {
      method: 'GET',
      url,
    };
  ...
  async getItemById(accountId, itemId, loadRelations = ["ItemShops", "Images", "Manufacturer", "CustomFieldValues", "CustomFieldValues.value"]): Promise<Item> {
    const url = `https://api.merchantos.com/API/Account/${accountId}/Item/${itemId}.json?load_relations=${querystring.escape(JSON.stringify(loadRelations))}`;
  ...
  getCustomers(accountId, customersSearchParams: CustomerSearchParams = {}): RetailApiCursor<Customer> {
