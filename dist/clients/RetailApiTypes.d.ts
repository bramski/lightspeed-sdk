export declare type SearchOperator = '=' | '>' | '>=' | '<' | '<=' | '><' | '!=' | '~' | '!~' | 'IN' | 'or';
export declare type SearchParam = [SearchOperator, string];
export declare type CustomerParams = 'firstName' | 'lastName' | 'dob' | 'archived' | 'title' | 'company' | 'companyRegistrationNumber' | 'vatNumber' | 'createTime' | 'timeStamp' | 'creditAccountID' | 'customerTypeID' | 'discountID' | 'taxCategoryID';
...
export declare type CustomFieldValue = {
...
export declare type TaxCategory = {
...
export declare type SalePayment = {
    amount: number;
    paymentTypeID: number;
    registerID: number;
    employeeID: number;
};
export declare type SalePaymentArrayType = {
    SalePayment: SalePayment;
};
export declare type SaleLine = {
    unitQuantity: number;
    unitPrice: number;
    itemID: number;
    taxClassID: number;
...
export declare type SaleLineArrayType = {
    SaleLine: SaleLine;
};
export declare type PostSale = Optional<Sale, 'saleID' | 'createTime' | 'updateTime' | 'timeStamp' | 'archived' | 'isTaxInclusive' | 'tax1Rate' | 'MetaData' | 'calcAvgCost' | 'calcDiscount' | 'calcFIFOCost' | 'calcNonTaxable' | 'calcPayments' | 'calcSubtotal' | 'calcTax1' | 'calcTax2' | 'calcTaxable' | 'calcTips' | 'calcTotal' | 'change' | 'discountID' | 'displayableSubtotal' | 'displayableTotal' | 'quoteID' | 'receiptPreference' | 'shipToID' | 'tax2Rate' | 'taxTotal' | 'ticketNumber' | 'tipEmployeeID' | 'tipEnabled' | 'total' | 'totalDue' | 'discountPercent' | 'voided' | 'completeTime' | 'referenceNumber' | 'referenceNumberSource'>;
